...
static int ngx_pg_parser_mod(ngx_pg_save_t *s, int32_t mod) {
...
static int ngx_pg_parser_auth(ngx_pg_save_t *s) {
...
static int ngx_pg_parser_rowval(ngx_pg_save_t *s, size_t len, const u_char *str) {
    ...
    if (!d->row->nelts) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!nelts"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_pg_row_t *row = &elts[d->row->nelts - 1];
    ngx_str_t *rowelts = row->str->elts;
    if (!row->str->nelts) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!nelts"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
...
static int ngx_pg_parser_close(ngx_pg_save_t *s) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%s", __func__);
    return s->rc;
}

static int ngx_pg_parser_column(ngx_pg_save_t *s, int16_t col) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i", col);
    ngx_pg_data_t *d = s->data;
    if (!d) return s->rc;
    ngx_pg_col_t *elts = d->col->elts;
    if (!d->col->nelts) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!nelts"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_pg_col_t *col_ = &elts[d->col->nelts - 1];
    col_->col = col;
    return s->rc;
}
...
static int ngx_pg_parser_cmd(ngx_pg_save_t *s, int32_t len) {
...
static int ngx_pg_parser_error(ngx_pg_save_t *s, int32_t len) {
    if (!len) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!len"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i", len);
    ngx_pg_data_t *d = s->data;
    if (!d) return s->rc;
...
static int ngx_pg_parser_col(ngx_pg_save_t *s, int32_t len) {
    if (!len) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!len"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i", len);
    ngx_pg_data_t *d = s->data;
    if (!d) return s->rc;
...
static int ngx_pg_parser_format(ngx_pg_save_t *s, int16_t fmt) {
...
static int ngx_pg_parser_idle(ngx_pg_save_t *s) {
...
static int ngx_pg_parser_inerror(ngx_pg_save_t *s) {
...
static int ngx_pg_parser_intrans(ngx_pg_save_t *s) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%s", __func__);
...
static int ngx_pg_parser_key(ngx_pg_save_t *s, int32_t key) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i", key);
    return s->rc;
}

static int ngx_pg_parser_method(ngx_pg_save_t *s, int32_t method) {
...
static int ngx_pg_parser_nbytes(ngx_pg_save_t *s, int32_t nbytes) {
...
static int ngx_pg_parser_optkey(ngx_pg_save_t *s, size_t len, const u_char *str) {
    if (!len) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!len"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%*s", (int)len, str);
    if (!s->opt->nelts) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!nelts"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_pg_key_val_t *elts = s->opt->elts;
    ngx_pg_key_val_t *kv = &elts[s->opt->nelts - 1];
    ngx_memcpy(kv->key.data + kv->key.len, str, len);
    kv->key.len += len;
    return s->rc;
}

static int ngx_pg_parser_parse(ngx_pg_save_t *s) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%s", __func__);
    return s->rc;
}

static int ngx_pg_parser_pid(ngx_pg_save_t *s, int32_t pid) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i", pid);
    s->pid = pid;
    return s->rc;
}

static int ngx_pg_parser_ready(ngx_pg_save_t *s) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%s", __func__);
    return s->rc;
}

static int ngx_pg_parser_secret(ngx_pg_save_t *s) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%s", __func__);
    return s->rc;
}

static int ngx_pg_parser_opt(ngx_pg_save_t *s, int32_t len) {
    if (!len) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!len"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i", len);
    ngx_pg_key_val_t *kv;
    if (!(kv = ngx_array_push(s->opt))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!ngx_array_push"); s->rc = NGX_ERROR; return s->rc; }
    ngx_memzero(kv, sizeof(*kv));
...
static int ngx_pg_parser_table(ngx_pg_save_t *s, int32_t tbl) {
...
static int ngx_pg_parser_row(ngx_pg_save_t *s, int32_t len) {
    if (!len) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!len"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i", len);
    ngx_pg_data_t *d = s->data;
    if (!d) return s->rc;
...
static int ngx_pg_parser_oid(ngx_pg_save_t *s, int32_t oid) {
...
static int ngx_pg_parser_oidlen(ngx_pg_save_t *s, int16_t len) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i", len);
    ngx_pg_data_t *d = s->data;
    if (!d) return s->rc;
    ngx_pg_col_t *elts = d->col->elts;
...
static int ngx_pg_parser_optval(ngx_pg_save_t *s, size_t len, const u_char *str) {
    if (!len) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!len"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%*s", (int)len, str);
    if (!s->opt->nelts) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!nelts"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_pg_key_val_t *elts = s->opt->elts;
...
static const pg_parser_settings_t ngx_pg_parser_settings = {
    ...
    .cmd = (pg_parser_int4_cb)ngx_pg_parser_cmd,
    ...
    .oidlen = (pg_parser_int2_cb)ngx_pg_parser_oidlen,
    .oid = (pg_parser_int4_cb)ngx_pg_parser_oid,
    .optkey = (pg_parser_len_str_cb)ngx_pg_parser_optkey,
    .opt = (pg_parser_int4_cb)ngx_pg_parser_opt,
    .optval = (pg_parser_len_str_cb)ngx_pg_parser_optval,
...
static ngx_int_t ngx_pg_peer_get(ngx_peer_connection_t *pc, void *data) {
    ...
    if (pc->connection) s = d->save = (ngx_pg_save_t *)((char *)pc->connection->pool + sizeof(*pc->connection->pool)); else {
        ...
        if (!(s->opt = ngx_array_create(c->pool, 1, sizeof(ngx_pg_key_val_t)))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_array_create"); return NGX_ERROR; }
...
static ngx_int_t ngx_pg_opt_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_pg_save_t *s = d->save;
    if (!s->opt) return NGX_OK;
    ngx_pg_key_val_t *elts = s->opt->elts;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_uint_t i;
    for (i = 0; i < s->opt->nelts; i++) if (name->len - sizeof("pg_opt_") + 1 == elts[i].key.len && !ngx_strncasecmp(name->data + sizeof("pg_opt_") - 1, elts[i].key.data, elts[i].key.len)) break;
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", i == s->opt->nelts ? "true" : "false");
    if (i == s->opt->nelts) return NGX_OK;
    v->data = elts[i].val.data;
    v->len = elts[i].val.len;
    v->valid = 1;
    v->no_cacheable = 0;
...
static ngx_int_t ngx_pg_col_mod_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_mod_") - 1, name->len - sizeof("pg_col_mod_") + 1);
...
static ngx_int_t ngx_pg_col_col_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_col_") - 1, name->len - sizeof("pg_col_col_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static ngx_int_t ngx_pg_col_fmt_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_fmt_") - 1, name->len - sizeof("pg_col_fmt_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static ngx_int_t ngx_pg_col_name_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_name_") - 1, name->len - sizeof("pg_col_name_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static ngx_int_t ngx_pg_col_tbl_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_tbl_") - 1, name->len - sizeof("pg_col_tbl_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static ngx_int_t ngx_pg_col_len_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_len_") - 1, name->len - sizeof("pg_col_len_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static const ngx_http_variable_t ngx_pg_variables[] = {
  ...
  { ngx_string("pg_col_col_"), NULL, ngx_pg_col_col_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  { ngx_string("pg_col_fmt_"), NULL, ngx_pg_col_fmt_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  { ngx_string("pg_col_len_"), NULL, ngx_pg_col_len_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  { ngx_string("pg_col_mod_"), NULL, ngx_pg_col_mod_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  ...
  { ngx_string("pg_col_tbl_"), NULL, ngx_pg_col_tbl_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
