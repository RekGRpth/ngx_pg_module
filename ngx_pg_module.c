...
static ngx_int_t ngx_pg_nvalues_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
...
static const ngx_http_variable_t ngx_pg_variables[] = {
  ...
  { ngx_string("pg_nvalues"), NULL, ngx_pg_nvalues_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE, 0 },
...
static ngx_int_t ngx_pg_out_csv_plain_handler(ngx_http_request_t *r, size_t len, const u_char *data) {
    ...
    if (plcf->out.header) {
        ...
        for (ngx_uint_t i = 0; i < d->field->nelts; i++) {
            ...
            if (plcf->out.quote && plcf->out.escape) {
                for (ngx_uint_t k = 0; k < elts[i].name.len; k++) {
    ...
    for (ngx_uint_t i = 0; i < d->value->nelts; i++) {
        ...
        for (ngx_uint_t j = 0; j < elts[i].str->nelts; j++) {
            ...
            if (!str[j].data) {
                ...
                if (plcf->out.quote) if (ngx_pg_add_response(r, sizeof(plcf->out.quote), &plcf->out.quote) != NGX_OK) return NGX_ERROR;
                if (plcf->out.quote && plcf->out.escape) {
                    for (ngx_uint_t k = 0; k < str[j].len; k++) {
