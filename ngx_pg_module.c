...
static int ngx_pg_parser_result_count(ngx_pg_save_t *s, uint16_t count) {
    ...
    ngx_memzero(result, sizeof(*result));
    ngx_str_t *str;
    if (ngx_array_init(result, r->pool, count, sizeof(*str)) != NGX_OK) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "ngx_array_init != NGX_OK"); s->rc = NGX_ERROR; return s->rc; }
    return s->rc;
}
...
static ngx_chain_t *ngx_pg_write_str(ngx_pool_t *p, uint32_t *size, size_t len, const u_char *data) {
    ngx_chain_t *cl;
    if (!(cl = ngx_alloc_chain_link(p))) { ngx_log_error(NGX_LOG_ERR, p->log, 0, "!ngx_alloc_chain_link"); return NULL; }
...
static ngx_chain_t *ngx_pg_write_byte(ngx_pool_t *p, uint32_t *size, size_t len, const u_char *data) {
    ngx_chain_t *cl;
    if (!(cl = ngx_alloc_chain_link(p))) { ngx_log_error(NGX_LOG_ERR, p->log, 0, "!ngx_alloc_chain_link"); return NULL; }
...
static ngx_chain_t *ngx_pg_write_opt(ngx_pool_t *p, uint32_t *size, size_t len, const u_char *data) {
    ngx_chain_t *cl;
    if (!(cl = ngx_alloc_chain_link(p))) { ngx_log_error(NGX_LOG_ERR, p->log, 0, "!ngx_alloc_chain_link"); return NULL; }
    if (!(cl->buf = ngx_create_temp_buf(p, len + sizeof(u_char)))) { ngx_log_error(NGX_LOG_ERR, p->log, 0, "!ngx_create_temp_buf"); return NULL; }
...
static ngx_chain_t *ngx_pg_query(ngx_pool_t *p, size_t len, const u_char *data) {
...
static ngx_chain_t *ngx_pg_parse(ngx_pool_t *p, size_t len, const u_char *data, ngx_array_t *argument) {
    ...
    if (!(cl = cl->next = ngx_pg_write_str(p, &size, len, data))) return NULL;
    if (!(cl = cl->next = ngx_pg_write_int2(p, &size, argument->nelts))) return NULL;
    ngx_pg_value_t *value = argument->elts;
    for (ngx_uint_t i = 0; i < argument->nelts; i++) if (!(cl = cl->next = ngx_pg_write_int4(p, &size, value[i].type))) return NULL;
    cl_size->buf->last = pg_write_int4(cl_size->buf->last, size);
    cl->next = NULL;
...
static ngx_chain_t *ngx_pg_bind(ngx_pool_t *p, ngx_array_t *argument) {
    ...
    ngx_pg_value_t *value = argument->elts;
    if (!(cl = cl->next = ngx_pg_write_int2(p, &size, argument->nelts))) return NULL;
    for (ngx_uint_t i = 0; i < argument->nelts; i++) {
...
static ngx_chain_t *ngx_pg_function(ngx_pool_t *p, uint32_t oid, ngx_array_t *argument) {
...
//        for (ngx_chain_t *cmd = ngx_pg_connect(r->pool, pscf ? pscf->connect : plcf->connect); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
...
//        }
        ...
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_flush(r->pool); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
        d->ready++;
    ...
    d->ready++;
    s->data = d;
    ngx_array_t argument = {0};
    if (plcf->cmd.argument) {
        ngx_pg_value_t *value;
        if (ngx_array_init(&argument, r->pool, plcf->cmd.argument->nelts, sizeof(*value)) != NGX_OK) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "ngx_array_init != NGX_OK"); return NGX_ERROR; }
        ngx_pg_arg_t *arg = plcf->cmd.argument->elts;
        for (ngx_uint_t i = 0; i < plcf->cmd.argument->nelts; i++) {
            if (!(value = ngx_array_push(&argument))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_array_push"); return NGX_ERROR; }
    ...
    if (plcf->cmd.function) {
        ...
        if (cl) cl->next = ngx_pg_function(r->pool, oid, plcf->cmd.argument ? &argument : NULL);
        else cl = u->request_bufs = ngx_pg_function(r->pool, oid, plcf->cmd.argument ? &argument : NULL);
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_function(r->pool, oid, plcf->cmd.argument ? &argument : NULL); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
    } else if (plcf->cmd.argument) {
        if (cl) cl->next = ngx_pg_parse(r->pool, plcf->cmd.sql.len, plcf->cmd.sql.data, &argument);
        else cl = u->request_bufs = ngx_pg_parse(r->pool, plcf->cmd.sql.len, plcf->cmd.sql.data, &argument);
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_parse(r->pool, plcf->cmd.sql.len, plcf->cmd.sql.data, &argument); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
        cl->next = ngx_pg_bind(r->pool, &argument);
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_bind(r->pool, &argument); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
        cl->next = ngx_pg_describe(r->pool);
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_describe(r->pool); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
        cl->next = ngx_pg_execute(r->pool);
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_execute(r->pool); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
        cl->next = ngx_pg_close(r->pool);
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_close(r->pool); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
        cl->next = ngx_pg_sync(r->pool);
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_sync(r->pool); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
    } else {
        if (cl) cl->next = ngx_pg_query(r->pool, plcf->cmd.sql.len, plcf->cmd.sql.data);
        else cl = u->request_bufs = ngx_pg_query(r->pool, plcf->cmd.sql.len, plcf->cmd.sql.data);
        while (cl->next) cl = cl->next;
//        for (ngx_chain_t *cmd = ngx_pg_query(r->pool, plcf->cmd.sql.len, plcf->cmd.sql.data); cmd; cmd = cmd->next) {
//            cl->buf = cmd->buf;
//            if (!(cl = cl->next = ngx_alloc_chain_link(r->pool))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_alloc_chain_link"); return NGX_ERROR; }
//        }
    }
...
static ngx_int_t ngx_pg_input_filter_init(ngx_http_request_t *r) {
    ...
    u->pipe->length = 0;
    if (r->cache) {
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%i", r->cache->header_start);
        ngx_event_pipe_t *p = u->pipe;
...
static ngx_int_t ngx_pg_nresults_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    v->len = snprintf(NULL, 0, "%li", d->result->nelts);
    if (!(v->data = ngx_pnalloc(r->pool, v->len))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pnalloc"); return NGX_ERROR; }
    v->len = ngx_snprintf(v->data, v->len, "%li", d->result->nelts) - v->data;
    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
    return NGX_OK;
...
static ngx_int_t ngx_pg_result_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    ngx_uint_t j = m;
    if (!result || j >= result->nelts) return NGX_OK;
    ngx_str_t *str = result->elts;
    v->len = str[j].len;
    v->data = str[j].data;
    v->valid = 1;
...
static char *ngx_pg_arg_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ngx_pg_loc_conf_t *plcf = conf;
    ngx_pg_arg_t *arg;
    if (!plcf->cmd.argument && !(plcf->cmd.argument = ngx_array_create(cf->pool, 1, sizeof(*arg)))) return "!ngx_array_create";
    if (!(arg = ngx_array_push(plcf->cmd.argument))) return "!ngx_array_push";
...
static ngx_command_t ngx_pg_commands[] = {
  ...
  { ngx_string("pg_sql"), NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_TAKE1, ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, cmd.sql), NULL },
