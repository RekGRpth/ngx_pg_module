...
static ngx_int_t ngx_pg_parser_atttypmod(ngx_pg_save_t *s, const void *ptr) {
    ...
    elts[d->col->nelts - 2].atttypmod = atttypmod;
    return s->rc;
}
...
static ngx_int_t ngx_pg_parser_typlen(ngx_pg_save_t *s, const void *ptr) {
    ...
    if (!d) return s->rc;
    ngx_pg_col_t *elts = d->col->elts;
    elts[d->col->nelts - 2].typlen = typlen;
    return s->rc;
}
...
static ngx_chain_t *ngx_pg_exit(ngx_pool_t *p) {
    ...
    cl_size->buf->last = pg_write_uint32(cl_size->buf->last, size);
    cl->next = NULL;
...
static ngx_chain_t *ngx_pg_write_uint16(ngx_pool_t *p, uint32_t *size, uint16_t uint16) {
    ngx_chain_t *cl;
    if (!(cl = ngx_alloc_chain_link(p))) { ngx_log_error(NGX_LOG_ERR, p->log, 0, "!ngx_alloc_chain_link"); return NULL; }
    ...
    if (size) *size += sizeof(uint16);
    return cl;
}

static ngx_chain_t *ngx_pg_write_uint32(ngx_pool_t *p, uint32_t *size, uint32_t uint32) {
    ngx_chain_t *cl;
    if (!(cl = ngx_alloc_chain_link(p))) { ngx_log_error(NGX_LOG_ERR, p->log, 0, "!ngx_alloc_chain_link"); return NULL; }
...
static ngx_chain_t *ngx_pg_bind(ngx_http_request_t *r) {
    ...
    if (!(cl = cl->next = ngx_pg_write_str(p, &size, sizeof("") - 1, (u_char *)""))) return NULL;
    if (!(cl = cl->next = ngx_pg_write_uint16(p, &size, 0))) return NULL;
    if (!(cl = cl->next = ngx_pg_write_uint16(p, &size, plcf->arg->nelts))) return NULL;
    for (ngx_uint_t i = 0; i < plcf->arg->nelts; i++) {
        if (elts[i].complex.value.data) {
            ...
            if (!(cl = cl->next = ngx_pg_write_uint32(p, &size, value.len))) return NULL;
            if (!(cl = cl->next = ngx_pg_write_byte(p, &size, value.len, value.data))) return NULL;
        } else {
            if (!(cl = cl->next = ngx_pg_write_uint32(p, &size, -1))) return NULL;
        }
    }
    if (!(cl = cl->next = ngx_pg_write_uint16(p, &size, 0))) return NULL;
    cl_size->buf->last = pg_write_uint32(cl_size->buf->last, size);
...
static ngx_int_t ngx_pg_col_atttypmod_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_atttypmod_") - 1, name->len - sizeof("pg_col_atttypmod_") + 1);
...
static ngx_int_t ngx_pg_col_columnid_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_columnid_") - 1, name->len - sizeof("pg_col_columnid_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static ngx_int_t ngx_pg_col_command_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
...
static ngx_int_t ngx_pg_col_format_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_format_") - 1, name->len - sizeof("pg_col_format_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static ngx_int_t ngx_pg_col_ncols_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
...
static ngx_int_t ngx_pg_col_tableid_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_tableid_") - 1, name->len - sizeof("pg_col_tableid_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static ngx_int_t ngx_pg_col_typlen_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_col_typlen_") - 1, name->len - sizeof("pg_col_typlen_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->col || i >= d->col->nelts) return NGX_OK;
    ngx_pg_col_t *elts = d->col->elts;
...
static const ngx_http_variable_t ngx_pg_variables[] = {
  { ngx_string("pg_col_atttypmod_"), NULL, ngx_pg_col_atttypmod_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  { ngx_string("pg_col_columnid_"), NULL, ngx_pg_col_columnid_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  { ngx_string("pg_col_command"), NULL, ngx_pg_col_command_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE, 0 },
  { ngx_string("pg_col_format_"), NULL, ngx_pg_col_format_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  { ngx_string("pg_col_name_"), NULL, ngx_pg_col_name_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  { ngx_string("pg_col_ncols"), NULL, ngx_pg_col_ncols_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE, 0 },
...
static ngx_chain_t *ngx_pg_write_opt(ngx_pool_t *p, uint32_t *size, size_t len, const u_char *str) {
    ...
    if (!(cl = ngx_alloc_chain_link(p))) { ngx_log_error(NGX_LOG_ERR, p->log, 0, "!ngx_alloc_chain_link"); return NULL; }
    if (!(cl->buf = ngx_create_temp_buf(p, len + sizeof(uint8_t)))) { ngx_log_error(NGX_LOG_ERR, p->log, 0, "!ngx_create_temp_buf"); return NULL; }
    for (ngx_uint_t i = 0; i < len; i++) cl->buf->last = pg_write_uint8(cl->buf->last, str[i] == '=' ? 0 : str[i]);
    cl->buf->last = pg_write_uint8(cl->buf->last, 0);
    if (size) *size += len + sizeof(uint8_t);
    return cl;
}
...
static char *ngx_pg_connect(ngx_conf_t *cf, ngx_command_t *cmd, ngx_chain_t **connect) {
    ...
    if (!(cl = cl->next = ngx_pg_write_uint32(cf->pool, &size, 0x00030000))) return NGX_CONF_ERROR;
...
static ngx_chain_t *ngx_pg_parse(ngx_pool_t *p, size_t len, const u_char *str, ngx_array_t *arg) {
    ...
    if (!(cl = cl->next = ngx_pg_write_str(p, &size, sizeof("") - 1, (u_char *)""))) return NULL;
    if (!(cl = cl->next = ngx_pg_write_str(p, &size, len, str))) return NULL;
    if (!(cl = cl->next = ngx_pg_write_uint16(p, &size, arg->nelts))) return NULL;
    ngx_pg_arg_t *elts = arg->elts;
    for (ngx_uint_t i = 0; i < arg->nelts; i++) if (!(cl = cl->next = ngx_pg_write_uint32(p, &size, elts[i].type))) return NULL;
    cl_size->buf->last = pg_write_uint32(cl_size->buf->last, size);
    cl->next = NULL;
...
static ngx_chain_t *ngx_pg_execute(ngx_pool_t *p) {
    ...
    if (!(cl = cl->next = cl_size = ngx_pg_alloc_size(p, &size))) return NULL;
    if (!(cl = cl->next = ngx_pg_write_str(p, &size, sizeof("") - 1, (u_char *)""))) return NULL;
    if (!(cl = cl->next = ngx_pg_write_uint32(p, &size, 0))) return NULL;
    cl_size->buf->last = pg_write_uint32(cl_size->buf->last, size);
    cl->next = NULL;
