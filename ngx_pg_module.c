...
static int ngx_pg_fsm_authentication_ok(ngx_pg_save_t *s) {
    ...
    if (pscf && pscf->queries.elts) {
        ngx_chain_t *cl;
        if (!(cl = u->request_bufs = ngx_pg_queries(r, &pscf->queries))) { s->rc = NGX_ERROR; return s->rc; }
        while (cl->next) cl = cl->next;
        if (!(cl->next = ngx_pg_queries(r, &plcf->queries))) { s->rc = NGX_ERROR; return s->rc; }
    } else {
        if (!(u->request_bufs = ngx_pg_queries(r, &plcf->queries))) { s->rc = NGX_ERROR; return s->rc; }
    }
    u->request_sent = 0;
    u->write_event_handler(r, u);
    ngx_connection_t *c = r->connection;
...
static char *ngx_pg_argument_output_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd, ngx_pg_query_t *query) {
    ...
    for (ngx_uint_t i = cmd->offset & ngx_pg_type_parse ? 3 : 2; i < cf->args->nelts; i++) {
        ...
        if (str[i].len > sizeof("output=") - 1 && !ngx_strncasecmp(str[i].data, (u_char *)"output=", sizeof("output=") - 1)) {
            ...
            switch ((query->output.type = e[j].value)) {
...
static char *ngx_pg_bind_loc_ups_conf(ngx_conf_t *cf, ngx_command_t *cmd, ngx_array_t *queries) {
    ngx_pg_query_t *query;
    if (!queries->elts && ngx_array_init(queries, cf->pool, 1, sizeof(*query)) != NGX_OK) return "ngx_array_init != NGX_OK";
    if (!(query = ngx_array_push(queries))) return "!ngx_array_push";
    ngx_memzero(query, sizeof(*query));
...
static char *ngx_pg_bind_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ngx_pg_loc_conf_t *plcf = conf;
    return ngx_pg_bind_loc_ups_conf(cf, cmd, &plcf->queries);
}

static char *ngx_pg_bind_ups_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ngx_pg_srv_conf_t *pscf = conf;
    return ngx_pg_bind_loc_ups_conf(cf, cmd, &pscf->queries);
}
...
static ngx_command_t ngx_pg_commands[] = {
  { ngx_string("pg_bind"), NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_1MORE, ngx_pg_bind_loc_conf, NGX_HTTP_LOC_CONF_OFFSET, ngx_pg_type_location|ngx_pg_type_bind|ngx_pg_type_output, NULL },
  { ngx_string("pg_bind"), NGX_HTTP_UPS_CONF|NGX_CONF_1MORE, ngx_pg_bind_ups_conf, NGX_HTTP_SRV_CONF_OFFSET, ngx_pg_type_upstream|ngx_pg_type_bind, NULL },
