...
static ngx_int_t ngx_pg_create_request(ngx_http_request_t *r) {
    ...
    if (plcf->function) {
        ...
        if (!(cl = u->request_bufs = ngx_pg_parse(r->pool, plcf->sql.len, plcf->sql.data, &arguments))) return NGX_ERROR;
        while (cl->next) cl = cl->next;
...
//    if (!r->cache && b->pos == b->last) b->pos = b->last = b->start;
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "s->rc = %d", s->rc);
