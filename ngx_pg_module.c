...
static int ngx_pg_fsm_copy_data(ngx_pg_save_t *s, uint32_t len) {
    ...
    ngx_pg_data_t *d = s->data;
    if (!d) return s->rc;
    if (!d->results->nelts) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!nelts"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
...
static int ngx_pg_fsm_copy_out_response(ngx_pg_save_t *s, uint32_t len) {
    ...
    ngx_http_request_t *r = d->request;
    ngx_array_t *results;
    if (!d->results && !(d->results = ngx_array_create(r->pool, 1, sizeof(*results)))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!ngx_array_create"); s->rc = NGX_ERROR; return s->rc; }
    if (!(results = ngx_array_push(d->results))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!ngx_array_push"); s->rc = NGX_ERROR; return s->rc; }
...
static int ngx_pg_fsm_data_row_count(ngx_pg_save_t *s, uint16_t count) {
    ...
    ngx_memzero(results, sizeof(*results));
    ngx_str_t *str;
    if (ngx_array_init(results, r->pool, count, sizeof(*str)) != NGX_OK) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "ngx_array_init != NGX_OK"); s->rc = NGX_ERROR; return s->rc; }
    return s->rc;
}
...
static int ngx_pg_fsm_result_val(ngx_pg_save_t *s, size_t len, const uint8_t *data) {
    ...
    ngx_array_t *results = d->results->elts;
    results = &results[d->results->nelts - 1];
    if (!results->nelts) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!nelts"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    ngx_str_t *str = results->elts;
    str = &str[results->nelts - 1];
    ngx_memcpy(str->data + str->len, data, len);
    str->len += len;
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%d,%d:%V", d->results->nelts - 1, results->nelts - 1, str);
    return s->rc;
}
...
static int ngx_pg_fsm_row_description(ngx_pg_save_t *s, uint32_t len) {
    ...
    if (!d) return s->rc;
    ngx_http_request_t *r = d->request;
    if (!(d->field.data = ngx_pnalloc(r->pool, len))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!ngx_pnalloc"); s->rc = NGX_ERROR; return s->rc; }
    return s->rc;
}
...
static int ngx_pg_fsm_row_description_beg(ngx_pg_save_t *s) {
    ...
    if (!d->fields) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!fields"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
    if (!(field = ngx_array_push(d->fields))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!ngx_array_push"); s->rc = NGX_ERROR; return s->rc; }
...
static int ngx_pg_fsm_row_description_count(ngx_pg_save_t *s, uint16_t count) {
    ...
    ngx_http_request_t *r = d->request;
    ngx_pg_field_t *field;
    if (!(d->fields = ngx_array_create(r->pool, count, sizeof(*field)))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!ngx_array_create"); s->rc = NGX_ERROR; return s->rc; }
    return s->rc;
}
...
static int ngx_pg_fsm_row_description_name(ngx_pg_save_t *s, size_t len, const uint8_t *data) {
    ...
    ngx_pg_field_t *field = d->fields->elts;
    field = &field[d->fields->nelts - 1];
    if (!field->name.data) field->name.data = d->field.data + d->field.len;
...
//        while (cl->next) cl = cl->next;
    ...
    if (plcf->arguments) {
        ...
        if (ngx_array_init(&arguments, r->pool, plcf->arguments->nelts, sizeof(*value)) != NGX_OK) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "ngx_array_init != NGX_OK"); return NGX_ERROR; }
        ngx_pg_arg_t *arg = plcf->arguments->elts;
        for (ngx_uint_t i = 0; i < plcf->arguments->nelts; i++) {
            if (!(value = ngx_array_push(&arguments))) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "!ngx_array_push"); return NGX_ERROR; }
            ngx_memzero(value, sizeof(*value));
            if (arg[i].type.value.data) {
                ngx_str_t str;
                if (ngx_http_complex_value(r, &arg[i].type, &str) != NGX_OK) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "ngx_http_complex_value != NGX_OK"); return NGX_ERROR; }
                ngx_int_t n = ngx_atoi(str.data, str.len);
                if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
                value->type = n;
            }
            if (arg[i].argument.value.data) if (ngx_http_complex_value(r, &arg[i].argument, &value->argument) != NGX_OK) { ngx_log_error(NGX_LOG_ERR, pc->log, 0, "ngx_http_complex_value != NGX_OK"); return NGX_ERROR; }
        }
    }
    if (plcf->function) {
        ngx_str_t value;
...
inline static ngx_flag_t ngx_pg_is_number(size_t len, const uint8_t *data) {
...
static ngx_int_t ngx_pg_out_csv_plain_handler(ngx_http_request_t *r) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    ...
    if (plcf->out.header && d->fields) {
        ...
        for (ngx_uint_t i = 0; i < d->fields->nelts; i++) {
            if (i) if (ngx_pg_out_handler(r, sizeof(plcf->out.delimiter), &plcf->out.delimiter) != NGX_OK) return NGX_ERROR;
            if (plcf->out.quote) if (ngx_pg_out_handler(r, sizeof(plcf->out.quote), &plcf->out.quote) != NGX_OK) return NGX_ERROR;
            if (plcf->out.quote && plcf->out.escape) for (ngx_uint_t k = 0; k < field[i].name.len; k++) {
                if (field[i].name.data[k] == plcf->out.quote) if (ngx_pg_out_handler(r, sizeof(plcf->out.escape), &plcf->out.escape) != NGX_OK) return NGX_ERROR;
                if (ngx_pg_out_handler(r, sizeof(field[i].name.data[k]), &field[i].name.data[k]) != NGX_OK) return NGX_ERROR;
            } else if (ngx_pg_out_handler(r, field[i].name.len, field[i].name.data) != NGX_OK) return NGX_ERROR;
            if (plcf->out.quote) if (ngx_pg_out_handler(r, sizeof(plcf->out.quote), &plcf->out.quote) != NGX_OK) return NGX_ERROR;
        }
    }
    if (!d->results) return NGX_OK;
    if (!d->results->nelts) return NGX_OK;
    ngx_array_t *results = d->results->elts;
    for (ngx_uint_t i = 0; i < d->results->nelts; i++) {
        if (i || plcf->out.header) if (ngx_pg_out_handler(r, sizeof("\n") - 1, (uint8_t *)"\n") != NGX_OK) return NGX_ERROR;
        ...
        for (ngx_uint_t j = 0; j < results[i].nelts; j++) {
            if (j) if (ngx_pg_out_handler(r, sizeof(plcf->out.delimiter), &plcf->out.delimiter) != NGX_OK) return NGX_ERROR;
            if (!str[j].data) {
                if (plcf->out.null.len) if (ngx_pg_out_handler(r, plcf->out.null.len, plcf->out.null.data) != NGX_OK) return NGX_ERROR;
            } else {
                ngx_flag_t string = plcf->out.string && ngx_pg_is_number(str[j].len, str[j].data);
                if (!string && plcf->out.quote) if (ngx_pg_out_handler(r, sizeof(plcf->out.quote), &plcf->out.quote) != NGX_OK) return NGX_ERROR;
                if (!string && plcf->out.quote && plcf->out.escape) for (ngx_uint_t k = 0; k < str[j].len; k++) {
                    if (str[j].data[k] == plcf->out.quote) if (ngx_pg_out_handler(r, sizeof(plcf->out.escape), &plcf->out.escape) != NGX_OK) return NGX_ERROR;
                    if (ngx_pg_out_handler(r, sizeof(str[j].data[k]), &str[j].data[k]) != NGX_OK) return NGX_ERROR;
                } else if (ngx_pg_out_handler(r, str[j].len, str[j].data) != NGX_OK) return NGX_ERROR;
                if (!string && plcf->out.quote) if (ngx_pg_out_handler(r, sizeof(plcf->out.quote), &plcf->out.quote) != NGX_OK) return NGX_ERROR;
            }
        }
    }
...
static ngx_int_t ngx_pg_out_csv_handler(ngx_http_request_t *r) {
    ...
    return ngx_pg_out_csv_plain_handler(r);
}
...
//    ngx_uint_t i = 0; for (u_char *p = b->pos; p < b->last; p++) ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%d:%d:%c", i++, *p, *p);
    ...
    if (s->rc == NGX_OK) {
        ...
        switch (plcf->out.type) {
            case ngx_pg_out_type_csv: s->rc = ngx_pg_out_csv_handler(r); break;
            case ngx_pg_out_type_plain: s->rc = ngx_pg_out_plain_handler(r); break;
            default: if (d->results && d->results->nelts) {
                ngx_array_t *results = d->results->elts;
                for (ngx_uint_t i = 0; i < d->results->nelts; i++) {
                    if (!results[i].nelts) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!nfields"); s->rc = NGX_HTTP_UPSTREAM_INVALID_HEADER; return s->rc; }
                    ngx_str_t *str = results[i].elts;
                    for (ngx_uint_t j = 0; j < results[i].nelts; j++) if (str[j].data) if (ngx_pg_out_handler(r, str[j].len, str[j].data) != NGX_OK) { s->rc = NGX_ERROR; return s->rc; }
                }
            } break;
        }
    }
...
static ngx_int_t ngx_pg_field_format_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_field_format_") - 1, name->len - sizeof("pg_field_format_") + 1);
...
static ngx_int_t ngx_pg_field_column_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_field_column_") - 1, name->len - sizeof("pg_field_column_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->fields || i >= d->fields->nelts) return NGX_OK;
    ngx_pg_field_t *field = d->fields->elts;
    v->len = snprintf(NULL, 0, "%d", field[i].column);
    if (!(v->data = ngx_pnalloc(r->pool, v->len))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pnalloc"); return NGX_ERROR; }
    v->len = ngx_snprintf(v->data, v->len, "%d", field[i].column) - v->data;
    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
    return NGX_OK;
...
static ngx_int_t ngx_pg_field_mod_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    if (!d->fields || i >= d->fields->nelts) return NGX_OK;
    ngx_pg_field_t *field = d->fields->elts;
    v->len = snprintf(NULL, 0, "%d", field[i].mod);
    if (!(v->data = ngx_pnalloc(r->pool, v->len))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pnalloc"); return NGX_ERROR; }
    v->len = ngx_snprintf(v->data, v->len, "%d", field[i].mod) - v->data;
    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
    return NGX_OK;
...
static ngx_int_t ngx_pg_field_name_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_field_name_") - 1, name->len - sizeof("pg_field_name_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->fields || i >= d->fields->nelts) return NGX_OK;
    ngx_pg_field_t *field = d->fields->elts;
...
static ngx_int_t ngx_pg_field_oid_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    if (!d->fields || i >= d->fields->nelts) return NGX_OK;
    ngx_pg_field_t *field = d->fields->elts;
    v->len = snprintf(NULL, 0, "%d", field[i].oid);
    if (!(v->data = ngx_pnalloc(r->pool, v->len))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pnalloc"); return NGX_ERROR; }
    v->len = ngx_snprintf(v->data, v->len, "%d", field[i].oid) - v->data;
    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
    return NGX_OK;
...
static ngx_int_t ngx_pg_field_table_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_field_table_") - 1, name->len - sizeof("pg_field_table_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->fields || i >= d->fields->nelts) return NGX_OK;
    ngx_pg_field_t *field = d->fields->elts;
    v->len = snprintf(NULL, 0, "%d", field[i].table);
    if (!(v->data = ngx_pnalloc(r->pool, v->len))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pnalloc"); return NGX_ERROR; }
    v->len = ngx_snprintf(v->data, v->len, "%d", field[i].table) - v->data;
    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
    return NGX_OK;
...
static ngx_int_t ngx_pg_nfields_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    if (u->peer.get != ngx_pg_peer_get) return NGX_OK;
    ngx_pg_data_t *d = u->peer.data;
    if (!d->fields) return NGX_OK;
    v->len = snprintf(NULL, 0, "%li", d->fields->nelts);
    if (!(v->data = ngx_pnalloc(r->pool, v->len))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pnalloc"); return NGX_ERROR; }
    v->len = ngx_snprintf(v->data, v->len, "%li", d->fields->nelts) - v->data;
    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
    return NGX_OK;
...
static ngx_int_t ngx_pg_nresults_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    if (!d->results) return NGX_OK;
    v->len = snprintf(NULL, 0, "%li", d->results->nelts);
    if (!(v->data = ngx_pnalloc(r->pool, v->len))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pnalloc"); return NGX_ERROR; }
    v->len = ngx_snprintf(v->data, v->len, "%li", d->results->nelts) - v->data;
    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
    return NGX_OK;
...
static ngx_int_t ngx_pg_result_get_handler(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    v->not_found = 1;
    ngx_http_upstream_t *u = r->upstream;
    if (!u) return NGX_OK;
    ...
    ngx_pg_data_t *d = u->peer.data;
    ngx_str_t *name = (ngx_str_t *)data;
    u_char *c = ngx_strlchr(name->data + sizeof("pg_result_") - 1, name->data + name->len, '_');
    if (!c) return NGX_OK;
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%*s", c - name->data - sizeof("pg_result_") + 1, name->data + sizeof("pg_result_") - 1);
    ngx_int_t n = ngx_atoi(name->data + sizeof("pg_result_") - 1, c - name->data - sizeof("pg_result_") + 1);
    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); return NGX_ERROR; }
    ngx_uint_t i = n;
    if (!d->results || i >= d->results->nelts) return NGX_OK;
    ngx_array_t *results = d->results->elts;
    results = &results[i];
    ...
    ngx_uint_t j = m;
    if (!results || j >= results->nelts) return NGX_OK;
    ngx_str_t *str = results->elts;
    v->len = str[j].len;
    v->data = str[j].data;
    v->valid = 1;
...
static const ngx_http_variable_t ngx_pg_variables[] = {
  ...
  { ngx_string("pg_field_name_"), NULL, ngx_pg_field_name_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
  ...
  { ngx_string("pg_nfields"), NULL, ngx_pg_nfields_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE, 0 },
  { ngx_string("pg_nresults"), NULL, ngx_pg_nresults_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE, 0 },
  ...
  { ngx_string("pg_result_"), NULL, ngx_pg_result_get_handler, 0, NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_PREFIX, 0 },
...
static char *ngx_pg_opt_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
...
static char *ngx_pg_out_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    ngx_uint_t i;
    for (i = 0; t[i].name.len; i++) if (t[i].name.len == str[1].len && !ngx_strncmp(t[i].name.data, str[1].data, str[1].len)) break;
    if (!t[i].name.len) return "must be \"plain\" or \"csv\"";
...
static ngx_command_t ngx_pg_commands[] = {
  ...
  { ngx_string("pg_opt"), NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_TAKE1, ngx_pg_opt_loc_conf, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, options), NULL },
  { ngx_string("pg_opt"), NGX_HTTP_UPS_CONF|NGX_CONF_TAKE1, ngx_pg_opt_ups_conf, NGX_HTTP_SRV_CONF_OFFSET, offsetof(ngx_pg_srv_conf_t, options), NULL },
  ...
  { ngx_string("pg_upstream_buffer_size"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.buffer_size), NULL },
  { ngx_string("pg_upstream_buffers"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE2, ngx_conf_set_bufs_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.bufs), NULL },
  { ngx_string("pg_upstream_busy_buffers_size"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.busy_buffers_size_conf), NULL },
  ...
  { ngx_string("pg_upstream_ignore_client_abort"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.ignore_client_abort), NULL },
  { ngx_string("pg_upstream_intercept_errors"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.intercept_errors), NULL },
  ...
  { ngx_string("pg_upstream_next_upstream_timeout"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.next_upstream_timeout), NULL },
  { ngx_string("pg_upstream_next_upstream_tries"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_num_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.next_upstream_tries), NULL },
  ...
  { ngx_string("pg_upstream_read_timeout"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.read_timeout), NULL },
  { ngx_string("pg_upstream_request_buffering"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.request_buffering), NULL },
  { ngx_string("pg_upstream_send_timeout"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.send_timeout), NULL },
  { ngx_string("pg_upstream_socket_keepalive"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.socket_keepalive), NULL },
#if (NGX_HTTP_CACHE)
  ...
  { ngx_string("pg_upstream_cache_background_update"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_background_update), NULL },
  { ngx_string("pg_upstream_cache_bypass"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE, ngx_http_set_predicate_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_bypass), NULL },
  { ngx_string("pg_upstream_cache_lock_age"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_lock_age), NULL },
  { ngx_string("pg_upstream_cache_lock"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_lock), NULL },
  { ngx_string("pg_upstream_cache_lock_timeout"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_lock_timeout), NULL },
  { ngx_string("pg_upstream_cache_max_range_offset"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_off_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_max_range_offset), NULL },
  { ngx_string("pg_upstream_cache_methods"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE, ngx_conf_set_bitmask_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_methods), &ngx_http_upstream_cache_method_mask },
  { ngx_string("pg_upstream_cache_min_uses"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_num_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_min_uses), NULL },
  { ngx_string("pg_upstream_cache_revalidate"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_revalidate), NULL },
  { ngx_string("pg_upstream_cache_use_stale"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE, ngx_conf_set_bitmask_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_use_stale), &ngx_pg_next_upstream_masks },
  { ngx_string("pg_upstream_cache_valid"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE, ngx_http_file_cache_valid_set_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.cache_valid), NULL },
  { ngx_string("pg_upstream_no_cache"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE, ngx_http_set_predicate_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, upstream.no_cache), NULL },
#endif
    ngx_null_command
};
