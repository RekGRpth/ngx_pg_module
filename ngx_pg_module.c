...
static int ngx_pg_parser_result_val(ngx_pg_save_t *s, size_t len, const u_char *data) {
    ...
    str->len += len;
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%i,%i:%V", d->results->nelts - 1, results->nelts - 1, str);
    return s->rc;
}
...
static ngx_chain_t *ngx_pg_parse(ngx_pool_t *p, size_t len, const u_char *data, ngx_array_t *arguments) {
...
static ngx_chain_t *ngx_pg_function(ngx_pool_t *p, uint32_t oid, ngx_array_t *arguments) {
...
static ngx_int_t ngx_pg_peer_get(ngx_peer_connection_t *pc, void *data) {
    ...
    if (plcf->function) {
        ...
        if (cl) cl->next = ngx_pg_query(r->pool, plcf->sql.len, plcf->sql.data);
        else cl = u->request_bufs = ngx_pg_query(r->pool, plcf->sql.len, plcf->sql.data);
        while (cl->next) cl = cl->next;
    }
    cl->next = ngx_pg_flush(r->pool);
    while (cl->next) cl = cl->next;
...
static ngx_command_t ngx_pg_commands[] = {
  ...
  { ngx_string("pg_opt"), NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_TAKE1, ngx_conf_set_str_array_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_pg_loc_conf_t, options), NULL },
