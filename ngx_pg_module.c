...
static int ngx_pg_fsm_authentication_md5_password(ngx_pg_save_t *s, uint32_t salt) {
    ...
    s->command = pg_command_state_authentication_md5_password;
    s->salt = salt;
    return s->rc;
}
...
static int ngx_pg_fsm_authentication_sasl(ngx_pg_save_t *s, uint32_t len) {
    ...
    s->command = pg_command_state_authentication_sasl;
    return s->rc;
}
...
static char *ngx_pg_argument_output_loc_conf(ngx_conf_t *cf, ngx_pg_loc_conf_t *plcf, ngx_flag_t output) {
    ngx_pg_query_t *query = plcf->queries.elts;
...
static char *ngx_pg_function_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    if (ngx_http_compile_complex_value(&ccv) != NGX_OK) return "ngx_http_compile_complex_value != NGX_OK";
    return ngx_pg_argument_output_loc_conf(cf, conf, 1);
}
